...
function getPracticeStats($practice, $attendees) {
    ...
    foreach ($attendees as $a) {
        ...
        if ($gen == 0) {
            ...
            // ★変更：参加や途中参なら、時間はどうであれ一律「1回 ($c = 1)」とカウントする
...
foreach ($practices as $p) {
    ...
    foreach ($stats['user_stats'] as $uid => $data) {
        if (isset($user_court_fee[$uid])) { 
            $user_court_fee[$uid] += $c_unit * $data['weight']; 
            $user_counts[$uid] += $data['count']; // ★回数を加算
        }
    }
    ...
    <div class="main-wrapper">
        ...
        <main class="content-body">
            <div class="main-card">
                ...
                <p style="color: #666; font-size: 0.9em;">※総参加回数: <?php echo $all_users_total_count; ?> 回 / 総ボール代: ¥<?php echo number_format($total_ball_fee); ?></p>

                <table class="practice-table" style="display:block; overflow-x:auto; white-space:nowrap;">
                    <thead><tr style="background:#4a86e8; color:white;"><th>代</th><th>名前</th><th>最終金額</th><th>ｺｰﾄ代</th><th>ﾎﾞｰﾙ代</th><th>雑費</th><th style="color:#ffcccc;">立替済(ﾏｲﾅｽ)</th><th>参加回数</th><th>操作</th></tr></thead>
                    <tbody>
                        ...
                        <tr style="<?php echo ($user_totals[$u['id']] < 0) ? 'background:#fff3f3;' : ''; ?>">
                            ...
                            <td style="font-size:0.85em; color:#666;"><?php echo $user_counts[$u['id']]; ?>回</td>
                            <td>
